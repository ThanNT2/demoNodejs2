...
export const login = ({ email, password }) => new Promise(async (resolve, reject) => {
    try {
        ...
        const isCheckd = response && bcrypt.compareSync(password, response.password)
        const token = isCheckd ? jwt.sign({ id: response.id, email: response.email, role_code: response.role_code }, process.env.JWT_SECRET, { expiresIn: '5d' }) : null
        resolve({
            err: token ? 0 : 1,
        ...
        resolve({
            err: 0,
            mes: 'register service completed'
        })
        return
    } catch (error) {
        reject(error)
